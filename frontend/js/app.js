...
function toastLoja(message, type = 'success') {
    const container = el('loja-alert');
    container.innerHTML = `
...
async function carregarCategorias() {
    ...
    try {
        ...
        if (!json.success)
            throw new Error((_a = json.message) !== null && _a !== void 0 ? _a : 'Erro ao carregar categorias.');
        return (_b = json.data) !== null && _b !== void 0 ? _b : [];
    }
    catch (erro) {
...
function popularSelectCategorias(selectId, categorias, comOpcaoTodas) {
...
async function carregarProdutos() {
    ...
    const paginacaoWrap = el('produtos-paginacao');
    const params = new URLSearchParams();
    if (filtroLoja.busca)
        params.set('busca', filtroLoja.busca);
    if (filtroLoja.categoriaId)
        params.set('categoria_id', String(filtroLoja.categoriaId));
    ...
    try {
        ...
        modoOffline = false;
        let produtos = (_b = json.data) !== null && _b !== void 0 ? _b : [];
        ...
        renderPaginacao((_c = json.paginacao) !== null && _c !== void 0 ? _c : null);
    }
    catch (erro) {
...
function renderPaginacao(meta) {
    const wrap = el('produtos-paginacao');
    ...
    wrap.querySelectorAll('.btn-pagina').forEach((btn) => {
        btn.addEventListener('click', () => {
            ...
            if (!novaPagina || novaPagina < 1 || novaPagina > meta.total_paginas)
                return;
            filtroLoja.pagina = novaPagina;
            void carregarProdutos();
            window.scrollTo({ top: el('painel-loja').offsetTop - 80, behavior: 'smooth' });
        });
    });
}
function anexarEventosDosCards(produtos) {
    const modalDetalhesEl = el('produtoModal');
...
function abrirFormularioProduto(produto) {
    ...
    form.reset();
    el('produtoFormTitulo').textContent = produto ? 'Editar produto' : 'Novo produto';
    el('campo-id').value = produto ? String(produto.id) : '';
    el('campo-nome').value = (_a = produto === null || produto === void 0 ? void 0 : produto.nome) !== null && _a !== void 0 ? _a : '';
    el('campo-categoria').value = produto ? String(produto.categoria_id) : '';
    el('campo-preco').value = produto ? String(toNumber(produto.preco_unitario)) : '';
    el('campo-estoque').value = produto ? String(produto.estoque) : '';
    el('campo-imagem').value = (_b = produto === null || produto === void 0 ? void 0 : produto.imagem) !== null && _b !== void 0 ? _b : '';
    el('campo-descricao').value = (_c = produto === null || produto === void 0 ? void 0 : produto.descricao) !== null && _c !== void 0 ? _c : '';
    el('produtoFormErro').classList.add('d-none');
    const modalEl = el('produtoFormModal');
    // @ts-ignore
...
async function salvarProduto(evento) {
    ...
    const erroBox = el('produtoFormErro');
    erroBox.classList.add('d-none');
    const idTexto = el('campo-id').value;
    const payload = {
        id: idTexto ? Number(idTexto) : undefined,
        nome: el('campo-nome').value.trim(),
        categoria_id: Number(el('campo-categoria').value),
        preco_unitario: Number(el('campo-preco').value),
        estoque: Number(el('campo-estoque').value),
        imagem: el('campo-imagem').value.trim() || null,
        descricao: el('campo-descricao').value.trim(),
    };
    const botaoSalvar = el('btn-salvar-produto');
    ...
    try {
        ...
        if (!resp.ok || !json.success) {
            throw new Error((_a = json.message) !== null && _a !== void 0 ? _a : 'Não foi possível salvar o produto.');
        }
        // @ts-ignore
        (_b = bootstrap.Modal.getInstance(el('produtoFormModal'))) === null || _b === void 0 ? void 0 : _b.hide();
        toastLoja((_c = json.message) !== null && _c !== void 0 ? _c : 'Produto salvo com sucesso.', 'success');
        filtroLoja.pagina = 1;
...
function abrirConfirmacaoExclusao(produto) {
    if (modoOffline) {
        toastLoja('Exclusão exige o backend PHP/MariaDB rodando (XAMPP). Veja o README.', 'warning');
        return;
    }
    produtoParaExcluir = produto;
    el('confirmExclusaoNome').textContent = produto.nome;
    const modalEl = el('confirmExclusaoModal');
    // @ts-ignore
    new bootstrap.Modal(modalEl).show();
}
async function confirmarExclusao() {
    ...
    if (!produtoParaExcluir)
        return;
    const botao = el('btn-confirmar-exclusao');
    botao.disabled = true;
    try {
        ...
        const json = (await resp.json());
        if (!resp.ok || !json.success) {
            throw new Error((_a = json.message) !== null && _a !== void 0 ? _a : 'Não foi possível excluir o produto.');
        }
        // @ts-ignore
        (_b = bootstrap.Modal.getInstance(el('confirmExclusaoModal'))) === null || _b === void 0 ? void 0 : _b.hide();
        toastLoja((_c = json.message) !== null && _c !== void 0 ? _c : 'Produto excluído.', 'success');
        await carregarProdutos();
    }
    catch (erro) {
...
// ------------------------------------------------------------
function inicializarFiltrosLoja() {
    const inputBusca = el('filtro-busca');
    const selectCategoria = el('filtro-categoria');
    const checkEstoque = el('filtro-estoque');
    const botaoNovo = el('btn-novo-produto');
    ...
    el('form-produto').addEventListener('submit', (e) => void salvarProduto(e));
    el('btn-confirmar-exclusao').addEventListener('click', () => void confirmarExclusao());
}
// ------------------------------------------------------------
...
async function carregarDashboard() {
    ...
    const rankingWrap = el('dashboard-ranking');
    const categoriasWrap = el('dashboard-categorias');
    ...
    try {
        ...
        renderRankingProdutos(por_produto !== null && por_produto !== void 0 ? por_produto : []);
        renderDistribuicaoCategorias(por_categoria !== null && por_categoria !== void 0 ? por_categoria : []);
 ...
 */
function renderRankingProdutos(porProduto) {
    const wrap = el('dashboard-ranking');
 ...
 */
function renderDistribuicaoCategorias(porCategoria) {
    const wrap = el('dashboard-categorias');
    if (porCategoria.length === 0) {
        wrap.innerHTML = '';
        return;
    }
    const totalGeral = porCategoria.reduce((soma, c) => soma + toNumber(c.faturamento_total), 0);
    const comPercentual = porCategoria.map((c) => (Object.assign(Object.assign({}, c), { percentual: totalGeral > 0 ? (toNumber(c.faturamento_total) / totalGeral) * 100 : 0 })));
